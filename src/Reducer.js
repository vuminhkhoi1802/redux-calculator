import { TOGGLE_PLUS_MINUS, SAVE_EXPRESSION, CALCULATE_RESULT, CLEAR_RESULT } from './Constants';
...
const rootReducer = (state = initialState, action) => {
	switch (action.type) {
		...
		case SAVE_EXPRESSION:
			const operator = [ '+', '-', '*', '/', '%' ];
			if (state.expression === '0' && operator.indexOf(action.payload) != -1) return state;
			return {
				...state,
